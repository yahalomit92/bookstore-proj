...
import OrdersCollection from "../controllers/OrdersController.js";
import BooksCollection from "../controllers/BooksController.js";
import AuthorsCollection from "../controllers/AuthorsController.js";
...
const ordersFunctions = {
	...
	getByHighestProfitDay: async (req, res) => {
		try {
			const result = await OrdersCollection.getByHighestProfitDay();
			res.status(200).json({
				day: result._id,
				profit: result.totalSales,
			});
		} catch (error) {
			res.status(500).json({ error: error.message });
		}
	...
	getTotalProfitBetweenDates: async (req, res) => {
		try {
			...
			const totalProfit =
				await OrdersCollection.getTotalProfitBetweenDates(from, to);
	...
	getMostPopularGenres: async (req, res) => {
		try {
			...
			const result = await OrdersCollection.getMostPopularGenres(
				from,
				to
			);
			res.status(200).json(result);
...
const reduceStock = async (books) => {
	...
	try {
		...
		for (const book of booksData) {
			...
			if (!orderBook)
				throw new Error(
					`Book with ID ${book._id.toString()} not found`
				);
